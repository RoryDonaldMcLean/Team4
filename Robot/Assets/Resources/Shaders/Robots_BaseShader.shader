...
////// Specular:
                ...
                float3 diffuseColor = lerp((_MainTex_var.rgb*_Highlights_var.rgb),(_MainTex_var.rgb*_Shadow_var.rgb),step(dot(lightDirection,i.normalDir),_ShadowFalloff)); // Need this for specular when using metallic
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : SV_Target {
                ...
                float4 _Highlights_var = tex2D(_Highlights,TRANSFORM_TEX(i.uv0, _Highlights));
                float4 _Shadow_var = tex2D(_Shadow,TRANSFORM_TEX(i.uv0, _Shadow));
                float3 diffColor = lerp((_MainTex_var.rgb*_Highlights_var.rgb),(_MainTex_var.rgb*_Shadow_var.rgb),step(dot(lightDirection,i.normalDir),_ShadowFalloff));
                float specularMonochrome;
                float3 specColor;
