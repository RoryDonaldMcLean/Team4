...
public class BeamPoint : MonoBehaviour
{
    ...
    void OnTriggerStay(Collider lightBeam)
    {
        ...
        if(lightBeam.transform.parent != pickedUpTransform)
        {
            ...
            float newZ = lightBeam.gameObject.GetComponent<Collider>().ClosestPointOnBounds(transform.position).z;
    ...
    {
        GameObject.FindGameObjectWithTag("Prism").GetComponent<PrismColourCombo>().TriggerExitFunction(beamColor);
